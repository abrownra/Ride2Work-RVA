...
// ─── PDF constants ────────────────────────────────────────────────────────────
export const W = 792, H = 612, M = 36, CW = W - M * 2
export const TCOLS = [
...
const LGRAY  = rgb(0.85, 0.87, 0.90)
const ALTROW = rgb(0.97, 0.97, 0.97)
const RED    = rgb(0.8,  0.1,  0.1)
const BLUE   = rgb(0.1,  0.35, 0.7)
const WHITE  = rgb(1,    1,    1)
const TOTBG  = rgb(0.22, 0.37, 0.62)
const MGRAY  = rgb(0.8,  0.8,  0.8)
const SUBGRAY= rgb(0.5,  0.5,  0.5)
const STATBG = rgb(0.95, 0.97, 1.0)

// ─── Helpers ──────────────────────────────────────────────────────────────────
function trunc(font, s, maxW, size) {
  if (!s) return ''
...
function hline(page, x, y, w, thick = 0.5, col = DGRAY) {
...
export function fmtC(n) { return n == null ? '\u2014' : '$' + Number(n).toFixed(2) }
function fmtM(n) { return n == null ? '\u2014' : Number(n).toFixed(1) }

export function drawTripHeader(page, y, font, bold) {
...
export function drawRiderGroupHeader(doc, page, y, riderName, font, bold, addPageFn) {
  const RH = 16
...
export async function drawTripRow(doc, page, y, trip, idx, font, bold, sigCache) {
  ...
  let x = M
  const diffVal = trip.rate_differential && Number(trip.rate_differential) > 0
    ? '+' + fmtC(trip.rate_differential) : '\u2014'
  const vals = [
    String(trip.trip_number ?? '-'), fmtD(trip.created_at),
    trip.drivers?.name || '-', trip.riders?.name || '-',
    trip.pickup_address || trip.start_address || '-', trip.dropoff_address || '-',
    fmtM(trip.miles_traveled), String(trip.rider_count ?? 1),
    fmtC(trip.rate_applied), diffVal, fmtC(trip.trip_total), '',
  ]
  for (let i = 0; i < TCOLS.length; i++) {
    ...
    if (c.key === 'signature') {
      if (hasSig) {
        try {
          const img = await doc.embedPng(sigCache[trip.signature_url])
          const sw = c.w - 4, sh = rowH - 6
          page.drawRectangle({ x: x + 2, y: y - rowH + 3, width: sw, height: sh, color: WHITE })
          page.drawImage(img, { x: x + 2, y: y - rowH + 3, width: sw, height: sh })
        } catch { page.drawText('[err]', { x: x + 2, y: textY, size: FS, font, color: RED }) }
      } else if (!trip.signature_url) {
        page.drawText('! MISSING', { x: x + 2, y: textY, size: FS, font: bold, color: RED })
      } else {
        page.drawText('-', { x: x + 2, y: textY, size: FS, font, color: DGRAY })
      }
    } else {
...
// ─── Prefetch signatures ──────────────────────────────────────────────────────
export async function buildSigCache(trips) {
  const sigCache = {}
  const urls = [...new Set(trips.map(t => t.signature_url).filter(Boolean))]
  await Promise.all(urls.map(async (url) => {
    try {
      const r = await fetch(url)
      sigCache[url] = r.ok ? new Uint8Array(await r.arrayBuffer()) : null
...
// ─── Report PDF ───────────────────────────────────────────────────────────────
export async function buildReport(trips, settings, sigCache, dateStart, dateEnd, title) {
  ...
  for (let i = 0; i < stats.length; i++) {
    const bx = M + i * (statW + 10)
    page.drawRectangle({ x: bx, y: y - statH, width: statW, height: statH, color: STATBG })
    page.drawText(stats[i].label, { x: bx + 8, y: y - 18, size: 9, font, color: SUBGRAY })
    page.drawText(stats[i].val,   { x: bx + 8, y: y - 40, size: 18, font: bold, color: BLUE })
  }
  y -= statH + 20
  ...
  for (let i = 0; i < drows.length; i++) {
    const [name, d] = drows[i]
    if (i % 2 === 1) page.drawRectangle({ x: M, y: y - TEXT_H, width: DTW, height: TEXT_H, color: ALTROW })
    dx = M; for (let j = 0; j < DCOLS.length; j++) { const v = j === 0 ? trunc(font, name, 196, FS) : j === 1 ? String(d.rides) : j === 2 ? String(d.riders) : fmtC(d.amt); page.drawText(v, { x: dx + 2, y: y - TEXT_H + 5, size: FS, font, color: DGRAY }); dx += DCOLS[j].w }
    hline(page, M, y - TEXT_H, DTW, 0.3, MGRAY); y -= TEXT_H
  }
...
// ─── Invoice PDF ──────────────────────────────────────────────────────────────
export async function buildInvoice(trips, settings, sigCache, weekStart, weekEnd, invoiceNumber) {
  const doc  = await PDFDocument.create()
  const font = await doc.embedFont(StandardFonts.Helvetica)
  const bold = await doc.embedFont(StandardFonts.HelveticaBold)
  let page = doc.addPage([W, H]); let y = H - M
  ...
  for (const line of [settings.company_address || '', settings.company_city_state_zip || '', `Phone: ${settings.company_phone || ''}`, `EIN: ${settings.company_ein || ''}`]) {
    page.drawText(line, { x: M, y, size: 9, font, color: DGRAY }); y -= 13
  }
  let ry = topY
  ...
  const meta = [
    ['Invoice #', String(invoiceNumber)], ['Project', settings.project_name || ''],
    ['Date', fmtFull(now)], ['Due Date', fmtFull(due)],
    ['Payable To', settings.invoice_payable_to || ''],
    ['For', `${settings.invoice_for_org || ''} - ${settings.invoice_for_dept || ''}`],
    ['Address', `${settings.invoice_for_address || ''}, ${settings.invoice_for_city_state_zip || ''}`],
    ['Period', `${fmtShort(weekStart)} - ${fmtShort(weekEnd)}`],
  ]
  for (const [lbl, val] of meta) {
    page.drawText(`${lbl}:`, { x: RX, y: ry, size: 8, font: bold, color: SUBGRAY })
    page.drawText(trunc(font, val, RW - 65, 8), { x: RX + 62, y: ry, size: 8, font, color: DGRAY }); ry -= 13
  }
  ...
  for (const [, rt] of sortedGroups) rt.sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime())
